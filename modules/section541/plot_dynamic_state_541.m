function fig = plot_dynamic_state_541(instance, planPrev, timelinePrev, stateBefore, tNow, cfg)
% plot_dynamic_state_541 - 动态状态图（冻结段=实线，待配送段=虚线）
% 修改日志
% - v1 2026-01-24: 初版：冻结/待配送分段绘制。
% - v2 2026-01-26: 图例顺序/颜色映射固定；节点标注中文化（配送中心/客户/充电站）。
% - v3 2026-01-27: 图例仅保留“路径1..K”，不显示节点/冻结/待配送图例。
% - v4 2026-01-29: 图例条目数量=实际绘制的路径数量（跳过空驶/无移动的 [0->0]）；并修复颜色重复（避免 lines() 超过默认色循环导致重色）。

    if nargin < 6, cfg = struct(); end %#ok<NASGU>
    ...
    K = numel(planPrev.detail);
    names = strings(K,1);
    for k = 1:K
        try names(k) = string(stateBefore.vehicles(k).name); catch, names(k) = "V"+k; end
    end
    ...
    for oi = 1:numel(order)
        k = order(oi);
        if k > numel(stateBefore.vehicles) || k > numel(planPrev.detail)
            continue;
        end
        route = planPrev.detail(k).route(:).';
        ...
        frozenIdx = stateBefore.vehicles(k).frozenSeqIndex;
        frozenIdx = max(1, min(frozenIdx, numel(route)));
        vname = char(string(stateBefore.vehicles(k).name));
        col = vehicle_color_541_(vname, k);

        if frozenIdx >= 2
            rr = route(1:frozenIdx);
            xs = coord(rr+1,1); ys = coord(rr+1,2);
        ...
        if frozenIdx < numel(route)
            rr = route(frozenIdx:end);
            xs = coord(rr+1,1); ys = coord(rr+1,2);
            plot(xs, ys, '--', 'Color', col, 'LineWidth', 2.0, 'HandleVisibility','off');
            if numel(xs) >= 2
                segs = [segs; [xs(1:end-1) ys(1:end-1) xs(2:end) ys(2:end)]]; %#ok<AGROW>
            end
        end
    ...
    title(sprintf('时刻 %s 配送状态', min_to_hhmm_(tNow)), 'Interpreter','none');
    if ~isempty(legH)
